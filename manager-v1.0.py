...
# * Important Stuff
masterKey = b'JdmfTuTejPHc06yURW4HZfrj7GueCIIxu5yjzzkXlDU='
...
masterDbSQL = '''CREATE TABLE IF NOT EXISTS master(
            ...
            fernetkey       TEXT               NOT NULL UNIQUE);'''
...
insertUser2Master = "INSERT INTO master(username, password, recoverycode, fernetkey) VALUES(?, ?, ?, ?);"
...
def encryption(file, akey):

    # Fernet Encryption Function
    fileName = file
    fernet = Fernet(akey)
...
def decryption(file, akey):
    
    # Fernet Decryption Function, 
    fileName = file
    fernet = Fernet(akey)
...
def genrate_encryption_key(flag):
    
    # Generates Fernet Keys for Users and Unique key for Encrypting Files
    ...
    if flag == 1:
        ...
        encryption(DBfile, key)
        encryption(keyFile, masterKey)
...
def view_data(uname, ukey):
    ...
    if choice == '1':
        ...
        xcrypt(userDBFile.replace('uname', uname))
        decryption(userDBFile.replace('uname', uname), ukey)
        ...
        if len(data) == 0:
            ...
            encryption(userDBFile.replace('uname', uname), ukey)
            xcrypt(userDBFile.replace('uname', uname))
...
def new_user(mKey):
    ...
    try:       
        cur.execute(insertUser2Master, (uname, passwd, recovCode, key))
        conn.commit()
        ...
        encryption(userDBFile.replace('uname', uname), key)
        xcrypt(userDBFile.replace('uname', uname))
    ...
    with open(f'{desktop}/{uname}.txt', 'w+') as f:
        f.write("-"*40)
        f.write("\n")
        f.write("| Username And Recovery Code\n")
        f.write("-"*40)
        f.write("\n")
        f.write("| NOTE: KEEP THIS FILE SAFE\n")
        f.write("-"*40)
        f.write("\n")
...
def main():
    ...
    if not os.path.isfile(keyFile):
        mKey = genrate_encryption_key(1)

    else:
        xcrypt(keyFile)
        decryption(keyFile, masterKey)
        f = open(keyFile, 'rb')
        mKey = f.read()
        f.close()
        encryption(keyFile, masterKey)
